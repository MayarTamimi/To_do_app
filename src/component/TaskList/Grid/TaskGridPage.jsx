  ...
  const TaskGridPage = () => {
    ...
    useEffect(() => {
      openDB(async () => {
        try {
          ...
          const fetchedTasks = (taskRes.data.tasks || taskRes.data).map((task) => ({
            ...task,
    ...
    const handleCreateTask = async (data) => {
      try {
        ...
        setTasks((prev) => [...prev, { ...newTask, statusId: newTask.status?.id || "" }]);
    ...
    return (
      <>
        ...
        <CreateTaskModel isOpen={openCreate} closeModal={closeCreateModal} onCreate={handleCreateTask} />
        <StatusModal
          isOpen={openStatusModal}
          onClose={() => setOpenStatusModal(false)}
          onSave={({ title, color }) => {
            axios
              .post(`${API_URL}/status`, { title, color }, { headers: { Authorization: sessionStorage.getItem("token") } })
              .then((res) => {
        ...
        <div className="task-grid">
          <div className="cards-grid-columns" style={{ display: "flex", gap: "16px" }}>
            {statuses.map((status) => {
                      ...
                      return (
                        <div key={task.id} className="task-card">
                          <button
                            ...
                            disabled
                            style={{
                              backgroundColor: lightenColor(status.color || "#fbe6ea", 0.85),
                              color: "#333",
                              border: "none",
                              padding: "6px 12px",
                              borderRadius: "6px",
